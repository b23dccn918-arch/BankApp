...
public class AdminAnalyticController implements Initializable {
	...
	@FXML
	private PieChart billStatusChart;
	...
	private void loadBillStatusChart() {
	    Map<String, Integer> data = analyticRepo.getBillStatusCount();
	    billStatusChart.getData().clear();
	    for (Map.Entry<String, Integer> entry : data.entrySet()) {
	        billStatusChart.getData().add(new PieChart.Data(entry.getKey(), entry.getValue()));
	    }
	}
