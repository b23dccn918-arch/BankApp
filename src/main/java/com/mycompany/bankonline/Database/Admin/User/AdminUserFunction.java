...
public class AdminUserFunction {
	...
	public static boolean banAccountByUserId(int userId) {
	    String sql = "UPDATE accounts SET status = 0 WHERE user_id = ? AND status = 1";
	...
	public static boolean unBanAccountByUserId(int userId) {
	    String sql = "UPDATE accounts SET status = 1 WHERE user_id = ?";
	    try (Connection conn = Connect.getConnection();
	         PreparedStatement stmt = conn.prepareStatement(sql)) {
	...
	public static boolean deleteAccountByUserId(int accountId) {
